...
brunch_download=$(curl https://api.github.com/repos/sebanc/brunch/releases | grep browser_download_url | grep "r$chrome_version" | tail -n1 |  cut -f 3 | sed 's/        "browser_download_url"://' | sed 's/"//' | sed "s/\n//" | sed 's/"//')
